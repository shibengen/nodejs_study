...
Keyword.prototype.tableName = db.db_prefix + 'keyword';
var _table_name=Keyword.prototype.tableName;
Keyword.prototype.find = function(_field, where, callback) {
    var _where='';
...
// 更新数据
Keyword.prototype.update = function(_field, _values, callback) {
...
Keyword.prototype.list = function( _field,where, callback) {
...
//add
Keyword.prototype.add = function(values, callback) {
    this.find('id', {field:['name'],value:[values[0]]}, function (error,result) {
        if (error && result==1) {
            callback(true, 'Query Error!');
            return;
        }else if(!error){
            callback(true, '关键词已存在');
            return;
        }
        var sql = "INSERT INTO  " + _table_name + " (`uid`,`name`,`create_time`) VALUES (?,?,?) ";
        //console.log(sql);
        db.pool.getConnection(function(err, conn) {
            if (err) {
                callback(true);
...
//删除
Keyword.prototype.delete = function(id, callback) {
